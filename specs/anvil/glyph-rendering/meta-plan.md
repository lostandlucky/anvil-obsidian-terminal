# Meta-plan: Glyph rendering — canvas renderer + bundled symbol font
...
2. **xterm renderer DOM → canvas switch** — load `@xterm/addon-canvas`. Today the plugin loads no renderer addon, so xterm silently falls back to its DOM renderer (`canvasCount: 0` confirmed by tonight's e2e). Switching to canvas (a) unblocks pixel-sampling visual tests that can RED on tofu vs glyph, (b) ships a 2-5× paint speedup, and (c) sidesteps WebGL's transparency caveats and texture-atlas invalidation cost — both relevant given the plugin's transparent-by-default background and live theme/font-change handlers. (WebGL was considered and rejected; canvas was the deliberate call.)
...
**IN:**
- Add a renderer addon to xterm (canvas) and wire its lifecycle into `XtermHost`.
...
**OUT:**
- WebGL renderer (rejected; see Context).
...
### Phase 1 — Glyph rendering: canvas renderer + bundled symbol font

**Goal.** Out-of-the-box, with no user font installation, nerd-font glyphs emitted by Claude Code, starship, lazygit, and similar CLIs render as glyphs (not tofu) inside the plugin's terminal — verified by a visual e2e test that samples canvas pixels and would have failed RED before the font landed. The renderer switches from DOM to canvas to make pixel sampling possible and to gain the paint-speed improvement; the bundled woff2 is loaded via `@font-face` + `unicode-range` so the user's primary font stack still drives ASCII / Unicode text rendering.
...
- **Conditional vs unconditional canvas-addon load.** The plugin's established convention for undocumented Obsidian APIs is feature-detect-then-fallback (`src/main.ts:214`, `src/dock/wrap-and-dock.ts:46`). The canvas addon isn't an undocumented API, but the same caution applies — Electron should always support it, but a graceful-degrade path back to the DOM renderer is cheap insurance.
- **Theme/font refresh and the canvas glyph cache.** xterm's canvas renderer flushes its glyph cache on `theme` and `fontFamily` change automatically per its docs. `phase-2-theming.e2e.ts` covers theme; the phase spec should add (or extend) a font-change e2e to cover the matching path explicitly.
...
- **Theme application is renderer-agnostic.** `host.applyTheme(theme)` already exists and routes through xterm's `ITheme`, which both DOM and canvas renderers consume identically. The renderer switch should not need to touch theme code; verify by running `phase-2-theming.e2e.ts` post-switch.

## Open questions to resolve in the phase spec

- Exact `unicode-range` set, derived from Symbols Nerd Font Mono's actual coverage.
- Conditional (feature-detect-then-fallback) vs unconditional canvas-addon load.
