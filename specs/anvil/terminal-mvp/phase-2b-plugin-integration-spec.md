...
- `phase-2a-pty-server-spec.md` and `phase-2a-completion.md` — the binary you're consuming, and what was actually built.
- `pty-server/PROTOCOL.md` — the WebSocket protocol you're implementing the client side of.
- `meta-plan.md` — Phase 2b entry, shared constraints, dependency map.
- `phase-1-completion.md` — especially "Notes for downstream phases" (interface shape, hotkey constraint, double-fit, bundle size).
- `../testing-approach.md` — boundary rule, RED/GREEN discipline, tricky e2e bits (especially focus management and async settling).
