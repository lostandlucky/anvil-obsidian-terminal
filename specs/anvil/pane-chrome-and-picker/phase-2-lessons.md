...
- **Restart behaviour (updated post-merge): NO tab-spec persistence.** The original Phase 3 plan serialized `{ tabs: Array<{shell, cwd, shellArgs}> }` via `getState` / `setState`, but AC8 was dropped during Phase 3 dogfooding — see `phase-3-workspace-container-completion-report.md` "Post-merge rescope". Matches VS Code: restart = fresh blank terminal if the container was open before, nothing otherwise. FI-005 (cross-session scrollback persistence) remains larger and separate.
