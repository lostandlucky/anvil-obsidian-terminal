...
- **Tab state serialization — SUPERSEDED.** Originally this note said Phase 3 owed `getState() / setState()` tab serialization. AC8 was dropped post-merge during Phase 3 dogfooding because the fire-and-forget ordering it required introduced a `PROMPT_EOL_MARK` regression, and the feature didn't match the user's expectation (VS Code doesn't do it either). Shipped behaviour: on view reconstruction (restart / workspace-plugin / popout), one blank terminal is created. See `phase-3-workspace-container-completion-report.md` "Post-merge rescope".
