...
## Phase: Phase 4 — First-fit cell measurement (BUG-003) — exec  [GREEN]

- Ran `/phase-exec Phase 4 no-audit` unattended from the on-disk skill copy (`~/.claude/skills/dev-workflow/skills/phase-exec/SKILL.md`; not registered with the Skill tool in this session). Triage recommendation B honored: implementation brief written (`specs/anvil/bug-sweep/phase-4-first-fit-measurement-brief.md`), sign-off auto-approved per contract, committed before RED.
- decisions:
  - [RESOLVED-AUTO] Verification mode → Mode B (code tests): pure scheduler unit-tested at Level 1; anchor at e2e (unit can't call `mount()` — xterm `open()` needs real layout).
  - [RESOLVED-AUTO] Corrective-fit vehicle → additive settle pass in `XtermHost.mount()` (fonts.ready + two frames), initial fit stays synchronous per the PROMPT_EOL_MARK coupling. Load-bearing discovery during brief prep: `FitAddon.fit()`/`proposeDimensions()` read the render service's CACHED cell dims, so at unchanged container size a bare corrective fit silently no-ops — BUG-003's written fix paths 1/2 could never have worked. The pass therefore forces a char-size re-measure via feature-detected `_core._charSizeService.measure()` (undocumented xterm seam; degrades to pre-fix behavior if it moves; static pin added to `undocumented-api-surface.test.ts` as a new vendor block, mirroring the Obsidian convention).
  - [RESOLVED-AUTO] No-churn criterion carried by xterm's own dedup (measure() fires no event on unchanged metrics; fit() only resizes on changed cols/rows) — pinned by a steady-state e2e case (zero resize events, stable cols), not by coalescer changes.
  - [RESOLVED-AUTO] RED repro vehicle → probe FontFace served over a deliberately slow local HTTP server (Node-side in the spec; CORS header for the app:// origin; Obsidian's CSP only restricts style-src — verified against the pinned binary's index.html), placed ahead of Georgia in the terminal font stack via `updateSettings({fontFamily})`; mount mid-load measures fallback metrics. Chosen over `document.fonts`-deregistration and `local()`-source variants for determinism (load timing fully controlled). Settings + probe face restored in afterEach.
  - [RESOLVED-AUTO] Root cause CONFIRMED at RED (amplified): reported cols 50 (Georgia fallback at mount) vs settled 83 (post-font-flow), reconciled only by forced re-measure — the font-flow theory held, no re-aim needed.
  - [RESOLVED-AUTO] Doc surfaces (architecture.md, terminal-container-view.md, ADR-0006) left untouched — the settle pass is internal; documented contracts unchanged at their altitude.
  - [RESOLVED-AUTO] known-bugs.md now has zero open entries — left in place with a pointer to the bug-sweep records rather than deleted (append-only convention).
